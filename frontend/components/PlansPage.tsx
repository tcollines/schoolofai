        ...
        try {
            ...
            await mysqlClient
                ...
                .insert({
                    ...
                    email: user.email || '',
                    message: `Corporate enrollment request submitted by student user ${user.name || 'Student'} (${user.email || ''}).`,
                    type: 'ENROLLMENT'
                });
        ...
        <div className="max-w-6xl mx-auto">
            <div className="flex items-center justify-between mb-8">
                ...
                {onBack && (
                    <button
                        onClick={onBack}
                        className="hidden sm:inline-block text-gray-500 dark:text-slate-400 hover:text-gray-900 dark:hover:text-white font-medium px-4 py-2 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-lg transition-colors"
                    >
                        ← {t('back_to_profile')}
                    </button>
                )}
            ...
            {(rating > 0 || localStorage.getItem(`schoolofai-rating-${user?.id || 'guest'}`)) ? (
                <div className="max-w-2xl mx-auto bg-white dark:bg-slate-900 border border-gray-150 dark:border-slate-800 rounded-3xl p-8 shadow-xl text-center space-y-6">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-slate-400 leading-relaxed">
                            {currentPlan === 'SPONSORED'
                                ? `Your learning account is fully sponsored. We're glad to have you here.`
                                : "You now have full, unrestricted access to all learning modules. We're excited to support your education journey!"
                            }
                        </p>
                    </div>

                    <div className="border-t border-gray-100 dark:border-slate-850 pt-6 space-y-3">
                        <h4 className="text-sm font-bold text-gray-800 dark:text-slate-300">Your Rating</h4>
                ...
                <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-8 max-w-6xl mx-auto px-4">
                ...
                <div className="p-8 rounded-2xl bg-white dark:bg-gray-900 border border-purple-200 dark:border-violet-600 text-gray-900 dark:text-white relative overflow-hidden hover:transform hover:-translate-y-1 hover:border-welile-purple dark:hover:border-purple-600 transition-all duration-300 shadow-xl flex flex-col justify-between">
                    ...
                    <div>
                        ...
                        <div className="text-4xl font-bold text-gray-900 dark:text-white mb-6">{formatAmount(100)}<span className="text-lg text-gray-500 dark:text-slate-400 font-normal"> one-time</span></div>
            ...
            {selectedPlanName && (
                <PaymentModal 
                    ...
                    amount={selectedPlanName === 'PLUS' ? formatAmount(19) : formatAmount(100)}
                    userId={user?.id}
                />
            )}
        </div>
