...
interface EventItem {
    ...
    location?: string;
    premiered?: boolean;
    visibility?: 'basic' | 'pro' | 'instructor';
}
...
const EventsPage: React.FC<EventsPageProps> = ({ courses = [] }) => {
    ...
    useEffect(() => {
        const fetchUserRole = async () => {
            const { data } = await mysqlClient.auth.getUser();
            if (data?.user) {
                const { data: profile } = await mysqlClient.from('profiles').select('role').eq('id', data.user.id).single();
                if (profile) {
    ...
    // Fetch admin events from database, synced with admin console updates
    useEffect(() => {
        const fetchEvents = async () => {
            const { data } = await mysqlClient.from('events').select('*');
            if (data) {
                setAdminEvents(data);
            }
        };
        ...
        fetchEvents();
        window.addEventListener('admin-events-update', fetchEvents);
        return () => {
            window.removeEventListener('admin-events-update', fetchEvents);
        };
    }, [courses]);
    ...
    useEffect(() => {
        const checkTimer = () => {
            ...
            for (const event of allEvents) {
                ...
                const visibility = event.visibility || 'basic';
                if (visibility === 'instructor' && userRole !== 'ADMIN') continue;
                if (visibility === 'pro' && userRole !== 'PRO' && userRole !== 'SPONSORED' && userRole !== 'ADMIN') continue;
    ...
    const filteredEvents = allEvents.filter(e => {
        ...
        const visibility = e.visibility || 'basic';
        if (visibility === 'instructor' && userRole !== 'ADMIN') {
            return false;
        }
        if (visibility === 'pro' && userRole !== 'PRO' && userRole !== 'SPONSORED' && userRole !== 'ADMIN') {
            return false;
        }

        // Plan filter select dropdown selection
        if (planFilter === 'basic' && visibility !== 'basic') {
            return false;
        }
        if (planFilter === 'pro' && visibility !== 'pro') {
            return false;
        }
                ...
                <div className="relative z-10 grid md:grid-cols-3 gap-6 items-center">
                    <div className="md:col-span-2 space-y-3 text-left">
                        ...
                        <h2 className="text-2xl lg:text-3xl font-extrabold tracking-tight">
                            {imminentEvent ? imminentEvent.title : 'No Live Sessions Scheduled'}
                        </h2>
                        <p className="text-sm text-slate-300 max-w-lg">
                            {imminentEvent ? imminentEvent.description : 'There are currently no live sessions or masterclasses scheduled. Check back later for updates.'}
                        </p>
                    </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-gray-150 dark:border-slate-800 pb-4">
                ...
                <div className="flex flex-wrap items-center gap-3 shrink-0">
                    ...
                    <select
                        ...
                        className="px-3 py-1.5 bg-white dark:bg-slate-900 text-gray-800 dark:text-white border border-gray-200 dark:border-slate-800 rounded-xl text-xs font-bold shadow-sm outline-none cursor-pointer"
                    >
                        <option value="all">All Plans</option>
                        <option value="basic">Basic Plan Events</option>
                        <option value="pro">Pro Plan Events</option>
                    </select>

                    {/* Filter Tabs */}
                    <div className="flex bg-gray-105 dark:bg-slate-800 p-1 rounded-xl gap-1">
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            {event.visibility && (
                                                <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border ${
                                                    event.visibility === 'pro'
                                                        ? 'bg-amber-50 dark:bg-amber-950/25 text-amber-700 dark:text-amber-400 border-amber-100 dark:border-amber-900/40'
                                                        : 'bg-violet-50 dark:bg-violet-950/25 text-violet-750 dark:text-violet-400 border-violet-100 dark:border-violet-900/40'
                                                }`}>
                                                    {event.visibility === 'pro' ? 'Pro Plan Only' : 'Basic Plan'}
                                                </span>
                                            )}
