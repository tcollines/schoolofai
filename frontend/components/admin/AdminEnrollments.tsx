    ...
    const filteredUsers = users
        ...
        .sort((a, b) => {
            ...
            if (sortBy === 'name') {
                ...
                const roleA = a.role === UserRole.INDIVIDUAL ? 'BASIC' : (a.role === UserRole.ADMIN ? 'ADMIN' : a.role);
                const roleB = b.role === UserRole.INDIVIDUAL ? 'BASIC' : (b.role === UserRole.ADMIN ? 'ADMIN' : b.role);
                                ...
                                <tr key={u.id} className="student-row hover:bg-gray-50/50 dark:hover:bg-slate-850/30 transition-colors">
                                    ...
                                    <td className="py-4 px-6">
                                        <span className={`px-3 py-1 rounded-full text-xs font-bold inline-block ${
                                            ...
                                            u.role === UserRole.ADMIN ? 'bg-rose-100 text-rose-700 dark:bg-rose-950/40 dark:text-rose-300 border border-rose-200 dark:border-rose-900/50' :
                                            u.role === UserRole.INSTRUCTOR ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-900/50' :
                                            'bg-teal-100 text-teal-700 dark:bg-teal-950/40 dark:text-teal-300 border border-teal-200 dark:border-teal-900/50'
                                        }`}>
                                                        ...
                                                        <div className="absolute right-0 mt-1.5 w-48 bg-white dark:bg-slate-950 border border-gray-200 dark:border-slate-850 rounded-2xl shadow-xl py-1.5 z-40 animate-in fade-in slide-in-from-top-1 duration-100 text-left">
                                                            ...
                                                            {[
                                                                ...
                                                                <button
                                                                    ...
                                                                    onClick={async () => {
                                                                        ...
                                                                        if (u.role === opt.value && opt.label !== 'Instructor') return;
                                                                        try {
                                                            ...
                                                            <button
                                                                onClick={async () => {
                                                                    ...
                                                                    if (u.role === UserRole.ADMIN) {
                                                                        if (!confirm(`Warning: You are deleting an Administrator account (${u.name || u.email}).\n\nAre you sure you want to proceed? This action is irreversible.`)) {
                                                                            return;
                                                                        }
                                                                    } else {
                            ...
                            <button
                                onClick={async () => {
                                    ...
                                    try {
                                        ...
                                        // 2. Update user profile role in profiles table to INSTRUCTOR
                                        await updateUserRole(selectedUpgradeUser.id, UserRole.INSTRUCTOR);
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="p-4 bg-gray-50 dark:bg-slate-850 border border-gray-100 dark:border-slate-800 rounded-2xl">
                                    ...
                                    <p className="text-sm font-bold text-gray-800 dark:text-white uppercase">{selectedDetailUser.role === 'ADMIN' ? 'ADMIN' : selectedDetailUser.role || 'BASIC'}</p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="p-4 bg-gray-50 dark:bg-slate-850 border border-gray-100 dark:border-slate-800 rounded-2xl">
                                    <p className="text-[10px] text-gray-400 dark:text-slate-500 uppercase font-extrabold mb-1">Stored Password / Method</p>
                                    <p className="text-sm font-bold text-indigo-650 dark:text-indigo-400 font-mono break-all select-all">
                                        {selectedDetailUser.password && !selectedDetailUser.password.startsWith('oauth-') && !selectedDetailUser.password.startsWith('google-')
                                            ? selectedDetailUser.password
                                            : 'OAuth Verified (No Password)'}
                                    </p>
                                </div>
                            </div>
