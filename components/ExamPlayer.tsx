...
const ExamPlayer: React.FC<ExamPlayerProps> = ({ quiz, courseTitle, onClose, onSubmit }) => {
    ...
    const startExam = async () => {
        if (quiz.preventTabChange && containerRef.current) {
            try {
                ...
                alert("Fullscreen is required for this strict exam.");
                return;
            }
        }
        setStarted(true);
    ...
    useEffect(() => {
        ...
        const handleVisibilityChange = () => {
            if (document.hidden && quiz.preventTabChange) {
                alert("Strict Mode Violation: You left the exam tab. Your exam has been automatically submitted.");
                autoSubmit();
            }
        };
        ...
        const handleFullscreenChange = () => {
            if (!document.fullscreenElement && quiz.preventTabChange) {
                alert("Strict Mode Violation: You exited full screen. Your exam has been automatically submitted.");
                autoSubmit();
            }
        };
    ...
    }, [started, isSubmitting, quiz.preventTabChange]);
    ...
    if (!started) {
        return (
            <div ref={containerRef} className="fixed inset-0 z-[100] bg-slate-900 flex items-center justify-center p-4">
    ...
    return (
        <div ref={containerRef} className="fixed inset-0 z-[100] bg-gray-50 flex flex-col h-screen">
            {/* Header */}
